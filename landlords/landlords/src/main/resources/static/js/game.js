...
//               this.players[loserASeat].pokerInHand = [];
                ...
                function gameOver() {
                    ...
                    if(this.whoseTurn==0){
                        ...
                        if(this.players[this.whoseTurn].isLandlord == this.players[0].isLandlord){
                            isWin = 1;
                        }
                        else{
                            isWin = 0;
                    ...
                    $.ajax({
                        ...
                        success: function(){
                            alert(self.players[self.whoseTurn].isLandlord ? "地主赢" : "农民赢");
                            PG.Socket.websocket.close();
                            PG.Socket.websocket = null;
                            self.state.start('MainMenu');
                            self.state.remove('Game');
                            self.state.add('Game', PG.Game);
            ...
            case PG.Protocol.RSP_RESTART:
                ...
                break;
            case 0:
                this.players[0].updateInfo(packet[1], packet[1]);
                break;
            default:
