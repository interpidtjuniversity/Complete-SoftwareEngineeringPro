...
from .gameutil import card_show, choose, game_init, new_choose
import jsonpickle
...
class Game(object):
    ...
    #新的game初始程序
    def new_game_start(self, cards, train):
        ...
        self.players[0].cards_left = self.playrecords.cards_left1 = p1_cards
        self.players[1].cards_left = self.playrecords.cards_left2 = p2_cards
        self.players[2].cards_left = self.playrecords.cards_left3 = p3_cards


    def get_card(self, card):
        '''
        提供卡片的level，返回对应卡片类型的卡片类
        '''
        if card == '1' or card == '2':
            rank = int(card) + 11
        elif card == '14' or card == '15':
            rank = int(card)
        else:
            rank = int(card) - 2
        return Card(card + '-a-' + str(rank))

    def my_new_card(self, cards, player_id):
        '''
        收到地主的牌的信息
        '''
        p_cards = []
        for c in cards:
            card = self.get_card(c)
            p_cards.append(card)
        self.players[player_id].cards_left += p_cards

    def use_new_card(self, cards, last_move_type):
        '''
        已经有牌被别人使用过了，这里记录一下
        '''
        p_cards = []
    ...
    #游戏进行    
    def new_get_next_move(self):
        self.i = 0

        self.last_move_type, self.last_move, self.end, self.yaobuqi = \
            self.players[self.i].new_play(self.playrecords)
        if self.end:
            self.playrecords.winner = self.i+1
        ...
        if not self.yaobuqi:
            for card in self.last_move:
                move.append(card.name)
        return move, self.last_move_type
...
class Player(object):
    ...
    def new_play(self, playrecords):
        '''
        在next_moves中选择出牌方法
        '''
        print(len(self.next_moves))
        for i in range(len(self.next_moves)):
            cards = []
            for k in self.next_moves[i]:
                cards.append(k.name) 
            print(self.next_move_types[i], cards)

        self.next_move_type, self.next_move = new_choose(
            next_move_types=self.next_move_types,
            next_moves=self.next_moves, 
            my_config=self.my_config,
            game=self.game,
            player_id=self.player_id)
