...
class Agent(object):
    ...
    def new_next_move(self):
        move, move_type = self.game.new_get_next_move()
    ...
    def new_game_init(self, cards, models=["rl","random","random"], train=True):
        self.models = models
        self.game = Game(self.models, self.my_config)
        self.game.new_game_start(cards, train)
    
    def add_new_card(self, cards, player_id):
        self.game.my_new_card(cards, player_id)

    def use_new_card(self, cards, last_move_type):
        self.game.use_new_card(cards, last_move_type)